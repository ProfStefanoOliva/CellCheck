...
The generated files are intentionally local-only artifacts. They provide a
repeatable dataset for manual checks of profile import, workbook reading,
report viewing and prudent `.xlsm` handling without using real student files.
"""
...
OUTPUT_DIR = Path(__file__).resolve().parents[1] / "manual_tests" / "generated"
...
def main() -> None:
    ...
    generated_files = [generate_workbook(spec, OUTPUT_DIR) for spec in WORKBOOK_SPECS]
